...
namespace THEngine
{
	...
	Vector3f& Vector3f::operator*=(const Quaternion& q)
	{
		XMVECTOR tmp = XMVector3Rotate(XMLoadFloat3(reinterpret_cast<const XMFLOAT3*>(this)),
			XMLoadFloat4(&q.quaternion));
		XMStoreFloat3(reinterpret_cast<XMFLOAT3*>(this), tmp);
		return *this;
	}

	Vector3f operator*(const Vector3f& v, const Quaternion& q)
	{
		XMVECTOR tmp = XMVector3Rotate(XMLoadFloat3(reinterpret_cast<const XMFLOAT3*>(&v)),
			XMLoadFloat4(&q.quaternion));
		Vector3f result;
		XMStoreFloat3(reinterpret_cast<XMFLOAT3*>(&result), tmp);
		return result;
	}
